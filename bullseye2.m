...
%
% modified for 16 segments AHA representation
% 
% usage example:
...
% 
% SSSR 2018/07/16
% Edited circoffset for AHA henry Aug 10/2018

function [h,t]=bullseye2(data,varargin);
...
while i <= length(varargin)
  switch lower(varargin{i})
    ...
    case 'aha'
      param.isAHA=varargin{i+1};   i=i+2;
    otherwise
...
if param.label, t=bull_labels(param, data); end
...
ind=1;
Nrad = size(data,2);
Ncirc = size(data,1)*ones(Nrad,1);
Ncircoffset = zeros(Nrad,1);
if(param.isAHA)
    Nrad = 3;
    Ncirc = [4 6 6];
    Ncircoffset = [135 150 150];
end

for k=1:Nrad
    for j=1:Ncirc(k)
        dtht=-diff(param.tht)/Ncirc(k);
        tht1=dtht*(j-1)+param.tht(1)-param.tht0-90+Ncircoffset(k);
        tht2=dtht* j   +param.tht(1)-param.tht0-90+Ncircoffset(k);
        
        dr=diff(param.rho)/Nrad;
...
function t=bull_labels(param, data);
...
%   ang=(60*(k-1)-param.tht0-30)*(pi/180);
  ang=(-60*(k-1)-param.tht0-60)*(pi/180) - 0.5;
%   [x,y]=pol2cart(ang,1.08*max(param.rho));
  % seg title
    [xt,yt]=pol2cart(ang,0.9*max(param.rho));
  % seg value
    [x,y]=pol2cart(ang,0.875*max(param.rho));
% 
%     t(k)=text(xt,yt,num2str(16-k+1));
  t(k)=text(x,y,num2str(data(k)));
  set(t(k), 'FontSize', 30)
  disp(get(t(k), 'FontSize'));
  if rem((ang*180/pi),360)>=180
    set(t(k),'rotation',ang*(180/pi)+90,'HorizontalAlignment','Center','FontSize',38,'FontName','Courier','FontWeight','Bold');
  elseif (ang*180/pi)==0 | (ang*180/pi)==360
    set(t(k),'rotation',ang*(180/pi)+90,'HorizontalAlignment','Center','FontSize',18,'FontName','Courier','FontWeight','Bold');
  else
    set(t(k),'rotation',ang*(180/pi)-90,'HorizontalAlignment','Center','FontSize',18,'FontName','Courier','FontWeight','Bold');
  end
end
...
%   [x,y]=pol2cart(ang,1.08*max(param.rho));
  % seg title
    [xt,yt]=pol2cart(ang,0.9*max(param.rho));
  % seg value
    [x,y]=pol2cart(ang,0.6*max(param.rho));
% 
%     t(k)=text(xt,yt,num2str(16-k+1));
  t(k)=text(x,y,num2str(data(k)), 'FontSize', 18);
  if rem((ang*180/pi),360)>=180
    set(t(k),'rotation',ang*(180/pi)+90,'HorizontalAlignment','Center','FontSize',18,'FontName','Courier','FontWeight','Bold');
  elseif (ang*180/pi)==0 | (ang*180/pi)==360
    set(t(k),'rotation',ang*(180/pi)+90,'HorizontalAlignment','Center','FontSize',18,'FontName','Courier','FontWeight','Bold');
...
for k=4:-1:1  
%   ang=(60*(k-1)-param.tht0-30)*(pi/180);
  ang=(-90*(k+2)-param.tht0-60)*(pi/180) - 0.5;
%   [x,y]=pol2cart(ang,1.08*max(param.rho));
disp(ang)
  % seg title
    [xt,yt]=pol2cart(ang,0.9*max(param.rho));
  % seg value
    [x,y]=pol2cart(ang,0.35*max(param.rho));
% 
%     t(k)=text(xt,yt,num2str(16-k+1));
  t(k)=text(x,y,num2str(data(k)));
  if rem((ang*180/pi),360)>=180
